...
在仓库根目录启动你平时使用的 YOLO 模式，并开启实时搜索：
...
2. 只有你决定新建知识库时才修改 `TASK_ID`；按当前关注更新 `RUN_MODE`、`RESEARCH_QUESTION`、长期学习
   目标、用户画像和其他课题级配置。默认 `RUN_MODE=research`，不会因为粘贴 Prompt 自动重写整个 Reader。
3. 复制代码块中的完整 Prompt，粘贴到 Codex，按一次 Enter。

`--enable goals` 用于显式启用持久 Goal，避免依赖不同 Codex CLI 版本的默认开关。

启用 Goals 时，Codex 可以为有界 run 维护持久 Goal；任务文件负责跨 run、上下文压缩或意外中断后的长期
恢复。`TASK_ID` 标识长期知识库，`RUN_ID` 与 Goal 共同标识一次截止/Token 合同。没有 Goal 工具时仍可在
当前会话执行并保存产物。
...
| `RUN_MODE` | `research` | `discovery` 只做候选；`reader-rewrite` 才做 Reader 修订 |
| `RESEARCH_QUESTION` | Agentic RL 当前研究问题 | 同一 ID 下可随关注点补充、收窄、转向或重写 |
...
| `USE_PERSISTENT_GOAL` | `true` | 无 Goal 工具时仍可在当前会话执行并保存 |
| `GOAL_TOKEN_BUDGET` | `2000000` | 每个 run/Goal 独立；活跃 run 不能改 |
| `READER_REWRITE_SCOPE` | `affected-documents` | 全量值只在显式 `reader-rewrite` 模式使用 |
| 外部额度阈值/检查/等待参数 | `$1` / `5` 分钟 / `60` 秒 | 通常不改 |
| 仓库、语言、深度、并行策略 | 当前仓库 / 中文 / deep / off | 通常不改 |

所有配置都有可运行默认值，不会因残留 `[必填]` 占位符而停下。恢复活跃 run 时不要修改它的
...
完成 `COMPLETION_EVIDENCE` 后可以用 `STOP_COMPLETE` 提前结束；也可以明确取消当前 run 或整个长期知识库。
初稿完成不是“研究永远完成”，只表示本次有界交付已保存。

`GOAL_TOKEN_BUDGET` 是当前 run 的 Codex Goal 总采样 Token 上限，用于自动续回合，预算用尽时当前
Goal/run 会结束；截止到达时也会封存该 run 并结束绑定的 Goal。完成验收也可以提前封存。下一时段用同一
`TASK_ID`、新的截止和 Token 预算创建 continuation run/Goal，继续写唯一知识库。它与
...
完成验收或用户明确取消当前 run 时可运行：

```bash
python3 tools/research_runtime.py close-run <task-id> --reason STOP_COMPLETE
# 用户取消时改为 STOP_USER
```

产品明确报告 Goal Token 自然耗尽时使用 `STOP_GOAL_TOKENS`。这些命令只关闭当前 run，不关闭长期课题。
...
- 截止时间和 Goal Token 是当前 run “不能越过”的资源边界；完成验收可以提前结束。后续研究用同一
  `TASK_ID` 创建 continuation run/Goal，不改写旧合同，不分裂主报告，也不因关注方向变化派生任务 ID。
