...
ANSWER = {
    "translation": "\n\n".join([
        "Transformer 遵循这种整体架构，前向状态 h⃗_t 经过编码器和解码器，并使用自注意力层。",
        "编码器把输入序列映射为连续表示，解码器根据该表示逐步生成输出序列。每一层都包含注意力机制与前馈网络。",
        "多头注意力允许模型同时关注不同位置的表示子空间，并在后续层中合并这些信息。",
        "训练时保留原始变量、公式、引用和数值；结构图中的输入先进入编码器，再将编码器表示传给解码器。",
        "表格比较 Base 与 Big 两种配置：Base 的质量为 27.3，Big 的质量为 28.4，数值保持原文。",
        "自注意力在同一序列内部建立位置之间的依赖关系，前向状态使用稳定的线性箭头表示。",
        "本页的图示、表格和正文共同说明了 Transformer 的编码器—解码器数据流与模型质量差异。",
    ]),
    "blocks": [
        {
            "type": "figure",
