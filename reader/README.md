...
使用范围是 Mac 上的桌面浏览器论文阅读。布局与验收围绕桌面窗口、浏览器缩放、鼠标和键盘操作，以及可拖动的正文/PDF/译文分栏展开。

## 本地阅读

```bash
cd /Users/4paradigm/Desktop/knowledge_factory/paper_search/reader
./serve.sh paper-research-base-knowledge-about-llm-20260717
```
...
PDF 阅读器顶部提供“概览”和“译文”开关；打开右侧译文栏后，可分别翻译本页、后台补齐全文和查看全文缓存状态。全文翻译只处理未缓存页面，全部缓存后按钮会变为“全文已缓存”并停止重复启动任务。“重新翻译”允许选择 `gpt-5.6-terra`、`gpt-5.6-sol` 或 `gpt-6-astra`，推理强度随模型切换，选择保存在当前浏览器。翻译按物理页直接调用无状态 Responses API，不创建或续接 Codex Session；跨页一致性由本地 glossary 和相邻页摘录提供。译文块使用“正文第 1 段”等自然标签，点击块会回查对应 PDF 页，将匹配区域置于 PDF 视口中央；放大后可按住 PDF 内容拖拽平移视图。知识问答运行时不会持有 PDF/翻译共用的长时锁；PDF 标签隐藏后暂停状态轮询，恢复时保持原页码并重新检查当前页缓存。缓存位于 `user-data/<TASK_ID>/translations.sqlite3`（需要兼容旧工具时可用 `restore.sh` 恢复为目录）。左侧项目栏可通过页面左上方的 `‹/›` 按钮收起或展开，状态会在浏览器本地保存。

## 当前页知识解析

解析按本页原文阅读顺序逐块输出，每块先用简单结论表达作者意思，再解释数据、机制和必要边界，末尾保留综合启发。完整原文摘录与出处默认折叠。请求同时附上当前页与前后各两页的原始 PDF 图片及带物理页码的文本（文档边界自动裁减），只有当前页用于生成解析块和区域。点击简短位置标签可框选原文区域：模型提供归一化图像区域，前端用 PDF.js 文本锚点核对位置；核对成功使用区域框，失败则退回精确摘录位置。图像区域是估计边界，不宣称像素精确；文本也无法匹配时明确提示。跨栏单元可使用多个框。

在 PDF 工具栏打开“解析”，选择模型、推理强度，再点击“解析本页知识”。只有这个按钮会触发模型调用；打开面板、停留和翻页都不会自动生成。已有结果可“重新解析”，失败时保留旧结果。解析与译文、聊天记录独立，模型选择分别保存在浏览器中。侧栏支持拖拽调整宽度和双击复位。

知识解析复用下述 Responses 端点和认证配置，必带当前物理页原始 PNG 和文本层，并提供前一页最多 14000 字、后一页最多 10000 字、首页最多 6000 字，以及邻近前文最多 32000 字（至多回溯 13 页）。辅助文本标记物理页，用于恢复定义、目标函数和实验背景，不冒充本页证据。输出核心命题、理解前提、按具体技术问题展开的图表/机制解析、本页得到的认识和证据边界。技术页通常为 1000～1800 中文字，复杂页可更长；纯参考文献页保持简短。旧版阅读评分、内容清单及标签不再显示。

提示词在 [`prompts/page-insight.md`](prompts/page-insight.md)，输出契约在 [`schemas/page-insight.schema.json`](schemas/page-insight.schema.json)。调整任一文件后，旧概览自动失效。缓存存入 `state.sqlite3` 的逻辑 `page-insights/` 路径，按 PDF 内容哈希、物理页、模型、推理强度和提示词/契约哈希区分。

`GET /api/pdf/page-insight?source_id=...&page=...&model=...&reasoning_effort=...` 只读缓存；`POST /api/pdf/page-insight` 使用相同字段生成，`force: true` 重新生成。POST 需要与其他 Reader 操作相同的来源和 `X-Reader-Token` 校验。默认使用 `gpt-5.6-terra / medium`，模型和推理强度沿用服务现有的允许列表。
...
知识问答、AI 修订和 PDF 翻译均可选择 `gpt-6-astra`，原有默认模型保持不变。
Astra 支持 `low/medium/high/xhigh/max` 推理强度；从旧模型的 `ultra` 切换到 Astra 时自动选用 `max`。
模型参数依据 [OpenAI Docs](https://developers.openai.com/api/docs/models/gpt-6-astra)。
...
- 技术追问默认按“明确回答、机制、具体例子、适当图解、可核验依据与边界”展开。后端传入选区的章节路径、所属章节（含表格/代码）、有界的文章正文及固定来源清单；源码问题要求主动查原始实现，不能只复述原段落。纯措辞修改仍保持用户指定范围。
- 修订提示词在 [`prompts/document-revision.md`](prompts/document-revision.md)，每次请求读取，修改该文件无需重启。正文上限为 24,000 字符（不是目标字数），交互图仍使用隔离的 `visual_html`。候选记录提示词哈希、上下文策略版本与截断情况，连续追问保留先前正文和图解。无效 JSON/图结构会自动尝试一次格式修复，保留已有讲解；再次失败则明确报错。上下文组装见 [`revision_context.py`](revision_context.py)，回归测试为 `tests/test_revision_context.py`。
